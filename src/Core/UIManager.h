...
namespace DuiLib {
	...
	{
		// Internally retained message
        //
		UIMSG_TRAYICON = WM_USER + 1,
        UIMSG_THREADMSG = WM_USER + 2,
...
#define UISTATE_CAPTURED     0x00000040
	...
	typedef struct UILIB_API tagTResInfo
	{
		...
		CStdStringPtrMap m_ImageHash; // SAVE TImageInfo
		CStdStringPtrMap m_AttrHash;
		CStdStringPtrMap m_StyleHash;
		CStdStringPtrMap m_DrawInfoHash; // SAVE TDrawInfo
	} TResInfo;
	...
	public:
		...
		SIZE GetInitSize();
		void SetInitSize(int cx, int cy, bool bDpiScale = true);
		RECT& GetSizeBox();
        ...
        static void SetResourceZip(int resourceID, const CDuiString& strResType = _T("ZIPRES"), LPCTSTR password = NULL);
		static void SetResourceType(UILIB_RESOURCETYPE nType);
	...
	private:
		...
		static UILIB_RESOURCETYPE m_nResType;
